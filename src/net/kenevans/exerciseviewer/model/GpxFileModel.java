...
public class GpxFileModel implements IConstants
{
    ...
    for(TrkType track : tracks) {
        ...
        for(Object object : objects) {
          ...
          if(object instanceof Oruxmapsextensions) {
            ...
            if(bpm != null && bpm.length() > 0) {
              res = getHrFromOruxMapsBpm(bpm, hrValsArray, hrTimeValsArray);
              if(res) {
      ...
      for(TrksegType trackSegment : trackSegments) {
        ...
        for(WptType tpt : trackPoints) {
          nTrackPoints++;
          xgcal = tpt.getTime();
          gcal = xgcal.toGregorianCalendar(TimeZone.getTimeZone("GMT"), null,
            null);
          ...
          if(prevTime != -1) {
            ...
            deltaLength = GpxUtils.greatCircleDistance(prevLat, prevLon, lat,
              lon);
            distance += deltaLength;
            deltaTime = time - prevTime;
            speed = deltaTime > 0?1000. * deltaLength / deltaTime:0;
            // Convert from m/sec to mi/hr
            speedValsArray.add(speed * GpxUtils.M2MI / GpxUtils.SEC2HR);
            speedTimeValsArray.add(time - Math.round(.5 * deltaTime));
          }
          prevTime = time;
          prevLat = lat;
          prevLon = lon;
          ...
          if(!usingOruxMapBpm && extensions != null) {
            ...
            for(Object object : objects) {
              ...
              if(object instanceof JAXBElement<?>) {
                ...
                if(node.getNodeName().contains("trackPointExtensionT")
                  && binder != null) {
                  JAXBElement<net.kenevans.gpxtrackpointextensionv2.TrackPointExtensionT> element = null;
                  try {
                    element = binder.unmarshal(node, trackPointExtensionTClass);
                  } catch(JAXBException ex) {
                    System.out.println("Cannot parse " + node.getNodeName());
                  }
                  if(element != null) {
      ...
      for(TrksegType trackSegment : trackSegments) {
        ...
        for(WptType waypoint : waypoints) {
          System.out.println("(" + waypoint.getLat() + "," + waypoint.getLon()
            + "," + waypoint.getEle() + ")" + " " + waypoint.getTime());
  ...
  public String getInfo() {
    ...
    info += String.format("Distance: %.2f mi", distance * GpxUtils.M2MI) + LS;
    ...
    if(nHrValues != 0) {
      ...
      info += String.format("HR Duration: %d hr %d min %d sec", hrDurationHours,
        hrDurationMin, hrDurationSec) + LS;
      stats = getTimeAverageStats(hrVals, hrTimeVals, -Double.MIN_VALUE);
      if(stats != null) {
        info += String.format("HR Min=%.0f HR Max=%.0f HR Avg=%.1f", stats[0],
          stats[1], stats[2]) + LS;
      } else {
        // Get simple average
        ...
        if(stats != null) {
          info += String.format(
            "HR Min=%.0f HR Max=%.0f HR Avg=%.1f" + " (Simple Average)",
            stats[0], stats[1], stats[2]) + LS;
        }
      }
    }
    if(speedVals.length != 0) {
      stats = getTimeAverageStats(speedVals, speedTimeVals, -Double.MIN_VALUE);
      if(stats != null) {
        info += String.format(
          "Speed Min=%.1f Speed Max=%.1f Speed Avg=%.1f mi/hr", stats[0],
          stats[1], stats[2]) + LS;
      } else {
        // Get simple average
        stats = getSimpleStats(speedVals, speedTimeVals, -Double.MIN_VALUE);
        if(stats != null) {
          info += String
            .format("Speed Min=%.1f Speed Max=%.1f Speed Avg=%.1f mi/hr"
      ...
      // Moving average
      // Convert from m/sec to mi/hr
      double noMoveSpeed = D_SPEED_NOT_MOVING * GpxUtils.M2MI / GpxUtils.SEC2HR;
      stats = getTimeAverageStats(speedVals, speedTimeVals, noMoveSpeed);
      if(stats != null) {
        info += String.format("  Moving Speed Avg=%.1f mi/hr", stats[2]) + LS;
      } else {
        // Get simple average
        stats = getSimpleStats(speedVals, speedTimeVals, noMoveSpeed);
        if(stats != null) {
          info += String.format(
            "  Moving Speed Avg=%.1f mi/hr" + " (Simple Average)", stats[2])
            + LS;
        }
      }
    ...
    if(eleVals.length != 0) {
      ...
      if(stats1 != null) {
        info += String.format("Ele Gain=%.0f Ele Loss=%.0f ft", stats1[0],
          stats1[1]) + LS;
      } else {
        info += LS;
      }
      ...
      if(stats != null) {
        info += String.format(
          "Ele Min=%.0f Ele Max=%.0f Ele Avg=%.1f ft" + " (Simple Average)",
          stats[0], stats[1], stats[2]) + LS;
      }
    }
    info += nTracks + " Tracks" + "        " + nSegments + " Segments:" + LS;
  ...
  /**
   * Gets the statistics from the given values and time values by averaging over
   * the values, not over the actual time.
   * 
   * @param vals
   * @param timeVals
  ...
  /**
   * Gets the statistics from the given values and time values by averaging over
   * the values weighted by the time.
   * 
   * @param vals
   * @param timeVals
   * @param omitBelow Do not include values below this one.
  ...
  public static double[] getTimeAverageStats(double[] vals, long[] timeVals,
    ...
    // + timeVals.length);
    if(vals.length != timeVals.length) {
      Utils.errMsg("getTimeAverageStats: Array sizes (vals: " + vals.length
        + ", timeVals: " + timeVals.length + ") do not match");
      return null;
    }
    int len = vals.length;
  ...
  /**
   * Gets the elevation statistics from the given values and time values. These
   * include elevation gain and loss.
   * 
   * @param vals
   * @param timeVals
   * @param omitBelow Do not include values below this one.
